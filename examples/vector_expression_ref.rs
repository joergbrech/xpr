...
impl<const N: usize> VecN<{ N }> {
    ...
    // wrap reference to self in terminal expression
    #[inline]
...
impl<'a, const N: usize> Fold for IthElement<'a, { N }> {
    ...
    #[inline]
    fn fold_term(&mut self, Term(v): &Term<&'a VecN<{ N }>>) -> f64 {
        v.0[self.0]
    }
}
...
where
    T: Foldable<IthElement<'a, { N }>, Output = f64>,
{
...
pub fn main() {
    ...
    let v = VecN::from(x1.as_xpr() + x2.as_xpr() + x3.as_xpr() + x4.as_xpr() + x5.as_xpr());
